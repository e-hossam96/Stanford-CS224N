...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = np.matmul(y_hat - y, outsideVectors)

    gradOutsideVecs = np.outer((y_hat - y), centerWordVec)
    # gradOutsideVecs = np.array([i * centerWordVec for i in (y_hat - y)])
    # END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    # defining first part of each equation
    loss = -np.log(y_hat)
    gradCenterVec = np.dot(y_hat - 1, outsideVectors[outsideWordIdx])
    gradOutsideVecs[outsideWordIdx] = np.dot(y_hat - 1, centerWordVec)

    # defining the last part of each equation
    for idx in indices[1:]:
        y_k_hat = sigmoid(-np.dot(outsideVectors[idx], centerWordVec))
        loss -= np.log(y_k_hat)
        gradOutsideVecs[idx] += np.dot(1.0 - y_k_hat, centerWordVec)
        gradCenterVec += np.dot(1.0 - y_k_hat, outsideVectors[idx])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentWordIdx = word2Ind[currentCenterWord]
    currentWordVec = centerWordVectors[currentWordIdx]

    for outsideWord in outsideWords:
        outsideWordIdx = word2Ind[outsideWord]
        currentLoss, centerGrad, outsideGrads = word2vecLossAndGradient(
            currentWordVec, outsideWordIdx, outsideVectors, dataset
        )
    
        loss += currentLoss
        gradCenterVecs[currentWordIdx] += centerGrad
        gradOutsideVectors += outsideGrads
